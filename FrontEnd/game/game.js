    ...
    if(gametype === 'local' && gameMode === 'player')
    {
        ...
        button.addEventListener('click', function() {
            ...
            if (playerName) {
                ...
                {
                    msg.innerText = 'The name must be 8 characters max and only letters';
                    return;
                }
                div.remove();
...
socket.onmessage = function(e) {
    ...
    if(data.event === 'draw')
    {
        ...
        else if(gametype === 'tournament')
        {
            ...
            document.getElementsByClassName('flex-container-finale')[0].remove();
        }
...
function letsStart()
{
    ...
    else if(gametype === 'tournament')
    {
        ...
        rightPlayerP.innerText = "PLAYER 2";
    ...
    if(gametype === 'local' || gametype === 'tournament')
        leftLetters.src = 'resources/letters.svg';
    else if(gametype === 'remote')
        leftLetters.src = 'resources/arrows.svg';
    leftLetters.className = 'movement-icon';
    let rightArrows = document.createElement('img');
    rightArrows.src = 'resources/arrows.svg';
    ...
    gameData.img.src = 'resources/background0.png';
...
function drawRightRoundedRect(ctx, x, y, width, height, color) {
    ...
    ctx.arcTo(x + width, y + height, x + width - radius, y + height, radius);  // Bottom-right corner (rounded)
...
function draw(currentTime) {
    ...
    if (deltaTime >= (1000 / fps)) {
        ...
        game.lineTo(gameData.width / 2, gameData.height - gameData.width * 0.1);
        game.lineWidth = 3;
        game.lineCap = "round";
        ...
        for (let i = ballTrail.length - 1; i >= 0; i--) {
            ...
            game.beginPath();
            game.arc(ballTrail[i].x, ballTrail[i].y, trailSize, 0, Math.PI * 2, true);
        ...
        game.shadowColor = gameData.ball.color;
        game.beginPath();
        game.arc(gameData.ball.x, gameData.ball.y, gameData.ball.radius, 0, Math.PI * 2, true);
        game.fillStyle = gameData.ball.color;
        game.fill();
        game.closePath();
        // Reset the shadow effect for other elements
        game.shadowBlur = 0;
        drawRightRoundedRect(game, gameData.xLeft, gameData.yLeft, gameData.widthPaddleSize, gameData.heightPaddleSize,'#00F0FF');
        // Draw right paddle with rounded corners
        drawLeftRoundedRect(game, gameData.xRight, gameData.yRight, gameData.widthPaddleSize, gameData.heightPaddleSize,'#F20089');
        //draw pause 
        if(pause)
        {
            // game.shadowBlur = 10;
            ...
            game.fillText("PAUSE", gameData.width * 0.45, gameData.height * 0.35);
            game.fillText("Press ESC to continue", gameData.width * 0.35, gameData.height * 0.43);
            ...
            game.shadowColor = '#BC00DD';
            game.fillStyle = "#6653DA";
            game.fillRect(exitButtonX, exitButtonY, exitButtonWidth, exitButtonHeight);
            game.shadowBlur = 0;

            game.fillStyle = "white";
            game.fillText("EXIT", exitButtonX + exitButtonWidth * 0.15, exitButtonY + exitButtonHeight * 0.6);
        ...
        if(gametype === 'local' || gametype === 'tournament')
        {
            if(!gameData.startTheGame)
            {
                if(gameData.i > 10 && gameData.i < 30)
                {
                    ...
                    game.fillStyle = "white";
                    game.font = `${gameData.fontSize}px audioWide`;
                    game.fillText("PRESS SPACEBAR TO START!", gameData.width * 0.35 , gameData.height * 0.45);
                    game.shadowBlur = 0;
                }
                if(gameData.i > 30)
                {
        ...
        else if(gametype === 'remote')
        {
            if(!gameData.startTheGame) {
                ...
                game.fillStyle = "white";
                game.font = `${gameData.fontSize}px audioWide`;
                game.fillText("THE GAME START AT :", gameData.width * 0.29 , gameData.height * 0.45);
                game.fillText(`${gameData.theCounter}`, gameData.width * 0.49 , gameData.height * 0.52);
                game.shadowBlur = 0;
            }
            if(!gameData.firstInstructions && firstInstructions)
                animationMovementButton();
...
function updateScore(player, score) {
    ...
    if (player === 'left') {
        if(document.getElementById('leftPlayerScore') && document.getElementById('leftBar'))
        {
            document.getElementById('leftPlayerScore').textContent = score;
            document.getElementById('leftBar').style.width = barWidthPercentage + '%';
        }

    } else if (player === 'right') {
        if(document.getElementById('rightPlayerScore') && document.getElementById('rightBar'))
        {
            document.getElementById('rightPlayerScore').textContent = score;
            document.getElementById('rightBar').style.width = barWidthPercentage + '%';
        }
    }
}
...
function animationMovementButton()
{
    if(gametype === 'local' || gametype === 'tournament')
    {
        ...
        if(leftkeys && rightkeys && leftscoreBar && rightscoreBar && leftPlayer && rightPlayer)
        {
